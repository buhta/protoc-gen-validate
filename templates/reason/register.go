...
func ReasonFilePath(f pgs.File, ctx pgsgo.Context, tpl *template.Template) *pgs.FilePath {
	...
	fileName := classNameFile(f) + "Validator.re"
...
func JavaMultiFilePath(f pgs.File, m pgs.Message) pgs.FilePath {
	fullPath := strings.Replace(javaPackage(f), ".", string(os.PathSeparator), -1)
	fileName := classNameMessage(m) + "Validator.re"
	filePath := pgs.JoinPaths(fullPath, fileName)
	return filePath
}
...
func classNameMessage(m pgs.Message) string {
	...
	// logic for reason, using "UnderscoreUnderscoreUnderscore" is an escape sequence seems to work with an extremely
...
func javaPackage(file pgs.File) string {
	// Explicit reason package overrides implicit package
